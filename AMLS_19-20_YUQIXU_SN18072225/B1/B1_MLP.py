...
def MLP_B1(training_images, training_labels, test_images, test_labels, val_images, val_labels):
    # learning parameters
    ...
    # training_images, training_labels, test_images, test_labels, val_images, val_labels =get_data_B1()
    ...
    with tf.Session() as sess:
        ...
        sess.run(init)
        # training_images = sess.run(training_images)
        # begin training loop ..
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy')
        plt.title('The graph of accuracy and val_accuracy (task B1)')
        plt.ylim([0, 0.7])
