...
def extract_features_labels():
    ...
    smiling_labels = {line.split(',')[0] : int(line.split(',')[2]) for line in lines[1:]}
    if os.path.isdir(images_dir):
        ...
        for img_path in image_paths:
            ...
            if features is not None:
                all_features.append(features[0:17])
                all_labels.append(smiling_labels[file_name+'.png'])

    landmark_features = np.array(all_features)
    smiling_labels = np.array(all_labels)  # simply converts the -1 into 0, so male=0 and female=1
